 ...
 * @author gopimac
 */
public class GenericPriorityQueueMax<K extends Comparable<K>, T> {

    private ArrayList<Pair<K, T>> list = new ArrayList<Pair<K, T>>();

    public GenericPriorityQueueMax() {
    }

    public GenericPriorityQueueMax(Collection<Pair<K, T>> col) {
        for (Pair<K, T> i : col) {
            this.insert(i);
        }
    }

    public Pair<K, T> getMaximum() {
        return list.get(0);
    }
    ...
    private void fix(int i) {
        ...
        if (rChild != -1 && list.get(rChild).getFirst().compareTo(list.get(max).getFirst()) == 1) {
            max = rChild;
        }
        if (max != i) {
