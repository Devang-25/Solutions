...
public class SplayTreeMedian {
	/*
	 * I forgot that my augmented BST is a splay tree, when accessing a node (select function)
	 * it need to perform the splay! (get function)
	 */
	static class Foo58 {
		void main() {
		...
		int select(int k) {
			...
			tree.get(key);	// just perform a splay, but very necessary!
			return key.x;			
		}
...
class TreeMapAug<K, V, A> {
	...
	/*
	 * Note that this function need to be re-invoked when tree is accessed ("splay" is invoked)
	 * because root will change
	 */
	...
	// return the node if find, or its parent if not find, or null if tree is null
