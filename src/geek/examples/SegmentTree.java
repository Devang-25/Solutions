...
public class SegmentTree {
	/*
	 * here we are updating only one, but mass update can be done easily.. just
	 * multiply the value in the interval by the overlap size. so far as fliping
	 * the coin is concerned, we toggle.
	 */
	public static void main(String[] args) {
		...
		int[] st = tree.constructST(arr, n);
		System.out.println(tree.getSum(st, n, 1, 3));

		// Update: set arr[1] = 10 and update corresponding segment
		// tree nodes
		tree.updateValue(arr, st, n, 1, 10);

		// Find sum after the value is updated
		System.out.println("Updated sum of values in given range "
				+ tree.getSum(st, n, 1, 3));
	}
	...
	private void updateValue(int arr[], int st[], int n, int i, int new_val) {
	...
	void updateValuesUtil(int[] st, int ss, int se, int qs, int qe, int diff,
			int index) {
		// Base Case: If the input index lies outside the range of this segment
		if (qe < ss || qs > se) {
			return;
		}
		int start = Math.max(qs, ss);
		int end = Math.min(qe, se);
		if (start < end) {
			st[index] = st[index] + diff * (end - start + 1);
			if (se != ss) {
				int mid = getMid(ss, se);
				updateValuesUtil(st, ss, mid, qs, qe, diff, 2 * index + 1);
				updateValuesUtil(st, mid + 1, se, qs, qe, diff, 2 * index + 2);
			}
		}
	}
	...
	// The function to update a value in input array and segment tree.
	// It uses updateValueUtil() to update the value in segment tree
	void updateValues(int arr[], int st[], int n, int i, int new_val) {
		// Check for erroneous input index
		if (i < 0 || i > n - 1) {
			System.out.println("Invalid Input");
			return;
